...
import { ClipboardCheck, X, Check, AlertCircle, Camera } from 'lucide-react';
...
const ConditionCheck = () => {
  ...
  const uploadPhotos = async () => {
    ...
    for (const file of files) {
      ...
      const fileName = `${booking.id}_${Math.random()}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/bookings/${id}/condition-check`, {
        method: 'POST',
        headers: {
      ...
      setError(err.message || 'Failed to submit condition check.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-gray-50 py-12 px-4">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-3xl font-extrabold text-gray-900">Pre-Rental Condition Check</h1>
          <p className="text-gray-500 mt-2">Document the physical state of <strong>{booking.product?.title}</strong> before use.</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-8">
          ...
          <div className="p-8 border-b border-gray-100 bg-gray-50/50">
            <h3 className="text-lg font-bold text-gray-900 mb-4">1. Verification Checklist</h3>
            <div className="space-y-3">
              {template.map((item) => (
                <label key={item.id} className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-xl cursor-pointer hover:border-primary transition-colors">
                  <div className={`w-6 h-6 rounded-md flex items-center justify-center border-2 flex-shrink-0 ${checklistValues[item.id] ? 'bg-primary border-primary' : 'border-gray-300'}`}>
          ...
          <div className="p-8 border-b border-gray-100">
            ...
            <p className="text-sm text-gray-500 mb-4">Please upload at least 3 photos (front, back, and close-up of any existing damage).</p>
          ...
          <div className="p-8">
            ...
            <textarea
              ...
              placeholder="List any existing scratches, missing minor accessories, or other remarks here..."
              className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all resize-none h-24 mb-6"
            ></textarea>
            ...
            <label className="flex items-start gap-4 p-4 bg-blue-50/50 border border-blue-100 rounded-xl cursor-pointer hover:border-blue-300 transition-colors">
              <div className={`w-6 h-6 rounded flex items-center justify-center border-2 mt-0.5 flex-shrink-0 ${agreed ? 'bg-primary border-primary' : 'border-gray-300 bg-white'}`}>
                {agreed && <Check size={14} className="text-white" />}
              </div>
              <p className={`text-sm ${agreed ? 'text-gray-900 font-medium' : 'text-gray-600'}`}>
                I confirm that the checklist, photos, and notes provided above accurately represent the physical condition of the item at the exact time of handover.
              </p>
        ...
        >
          {submitting ? 'Submitting & Securing...' : 'Submit Condition Report'}
        </Button>
      </div>
    </div>
  );
